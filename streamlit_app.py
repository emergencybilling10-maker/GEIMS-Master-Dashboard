...
for a in alerts:
    with st.container():
        st.markdown(f"<div style='background-color: #FFEBEE; border: 2px solid #FF5252; padding: 10px; border-radius: 5px; margin-bottom: 5px;'><b>🚨 TODAY'S BOOKING: {a.get('name', 'N/A')}</b></div>", unsafe_allow_html=True)
...
with st.expander("📋 MANAGE PATIENT REQUESTS", expanded=True):
    ...
    with st.form("new_req", clear_on_submit=True):
        ...
        p_fr = c2.selectbox("FROM", ["CCU", "EMERGENCY", "DELUXE", "PVT", "SEMI PVT", "HDU", "OPD", "ICU", "WARD", "LR", "OTHER"])
    ...
    if req_list:
        ...
        for idx, r in enumerate(req_list):
            # Display current actual status from database [FIXED LOGIC]
            curr_status = r.get('status', 'WAITING')
            bed_display = r.get('bed_no', '')
            ...
            r_cols[6].write(r.get('remark', '-')); r_cols[7].write(bed_display if bed_display else "-")
            
            color_map = {"DONE": "green", "CANCELLED": "red", "GEN-WARD ALLOTTED": "blue", "HOLD": "purple", "WAITING": "orange"}
            color = color_map.get(curr_status, "black")
            r_cols[8].markdown(f"<span style='color:{color}; font-weight:bold;'>{curr_status}</span>", unsafe_allow_html=True)
            
            if curr_status == "DONE":
                slip = f"""====================================\n      G.E.I.M.S (Bed Management)\n      BED ALLOTMENT SLIP\n====================================\nDATE: {today_date_str}\nPATIENT: {r['name']}\n------------------------------------\nBED:  {bed_display}\n===================================="""
                r_cols[9].download_button("🖨️", data=slip, file_name=f"Slip_{r['name']}.txt", key=f"rec_{r['ID']}")
...
with st.sidebar:
    ...
    if st.text_input("Password", type="password") == "GeimsAdmin99":
        ...
        # 🛠️ FIXED MODIFICATION HUB
        st.subheader("🛠️ Modify Record")
        if req_list:
            p_map = {f"{r['name']} ({r.get('status')})": r['ID'] for r in req_list}
            target_label = st.selectbox("Select Patient", list(p_map.keys()))
            tid = p_map[target_label]
            tdata = next(r for r in req_list if r['ID'] == tid)

            new_status = st.selectbox("Update Status to:", ["WAITING", "DONE", "HOLD", "CANCELLED", "GEN-WARD ALLOTTED"], index=["WAITING", "DONE", "HOLD", "CANCELLED", "GEN-WARD ALLOTTED"].index(tdata.get('status', 'WAITING')))
            new_bed = st.text_input("Update Bed ID:", value=tdata.get('bed_no', ''))
            
            if st.button("🔥 APPLY CHANGES"):
                # Clean up previous bed if it's changing
                old_bed = tdata.get('bed_no')
                if old_bed and old_bed != new_bed and old_bed in all_bed_ids:
                    db.collection("beds").document(old_bed).set({"status": "VACANT", "patient": ""})
                
                # If new status is DONE or Bed is added, update live bed
                if new_bed and new_bed in all_bed_ids:
                    db.collection("beds").document(new_bed).set({"status": "ALLOTTED", "patient": tdata['name']})

                # FORCE Update Request record
                db.collection("bed_requests").document(tid).update({
                    "status": new_status,
                    "bed_no": new_bed
                })
        ...
        # ↕️ SWITCH POSITIONS
        st.divider(); st.subheader("↕️ Move Order")
        if len(req_list) >= 2:
            p1_name = st.selectbox("Patient 1", [r['name'] for r in req_list], key="reorder_p1")
            p2_name = st.selectbox("Patient 2", [r['name'] for r in req_list], key="reorder_p2")
        ...
        # ⚙️ MANUAL BED UPDATE
        st.divider(); st.subheader("⚙️ Live Bed Manual")
        m_bed = st.selectbox("Select Bed", all_bed_ids); m_stat = st.selectbox("Status", ["VACANT", "BOOKED", "ALLOTTED", "DISCHARGE", "MAINTENANCE", "RESTRICTED"]); m_name = st.text_input("Name")
